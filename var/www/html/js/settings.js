...
async function loadUpdateStatus() {
    try {
        const res = await fetch(`${window.CONFIG.API_BASE}/update/status`);
        const data = await res.json();

        if (data.status !== 'success') {
            document.getElementById('update-status-text').textContent = 'Error loading status';
        ...
        const cvEl = document.getElementById('current-version');
        const lastCheckedEl = document.getElementById('last-checked');
        if (cvEl) cvEl.textContent = (current.version || current.commit || 'unknown');
        if (lastCheckedEl) lastCheckedEl.textContent = (cfg.last_check ? new Date(cfg.last_check).toLocaleString() : 'Never');

        const autoToggle = document.getElementById('auto-update-toggle');
        if (autoToggle) {
            autoToggle.checked = !!cfg.auto_update_enabled;
        }

        const statusEl = document.getElementById('update-status-text');
        if (statusEl) statusEl.textContent = 'Ready';
    } catch (err) {
        console.error('loadUpdateStatus error', err);
        const statusEl = document.getElementById('update-status-text');
...
async function checkForUpdates() {
    const statusEl = document.getElementById('update-status-text');
    const availSection = document.getElementById('update-available-section');
    ...
    try {
        ...
        const res = await fetch(`${window.CONFIG.API_BASE}/update/check`);
        if (!res.ok) {
        ...
        if (info.available) {
            if (infoText) {
                const message = info.latest_message || 'A new version is available';
                const date = info.latest_date ? new Date(info.latest_date).toLocaleString() : '';
                infoText.textContent = `${message}${date ? ` — ${date}` : ''}`;
            }
            if (availSection) availSection.classList.remove('hidden');
            if (statusEl) statusEl.textContent = `Update available (${info.commit_count || 1} commits)`;
        } else {
            if (statusEl) statusEl.textContent = 'System is up to date';
            if (availSection) availSection.classList.add('hidden');
        }
...
async function triggerUpdate() {
    ...
    try {
        ...
        const res = await fetch(`${window.CONFIG.API_BASE}/update/trigger`, {
            method: 'POST',
        ...
        if (!res.ok || data.status !== 'success') {
            const message = (data && data.message) ? data.message : `Update failed (HTTP ${res.status})`;
            throw new Error(message);
        }

        if (progressText) progressText.textContent = 'Update applied successfully. Restarting services...';

        setTimeout(() => {
            window.location.reload();
        }, window.CONFIG.UPDATE_PAGE_RELOAD_DELAY || 2000);
    } catch (err) {
        console.error('triggerUpdate error', err);
        if (progressText) progressText.textContent = `Update error: ${err.message}`;
